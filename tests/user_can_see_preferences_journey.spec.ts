...
test.describe("Account Preferences", () => {
  test("Should be able navigate to account preferences when logged in", async ({
    page,
  }) => {
  ...
  test("Should not see account preference icon when not logged in", async ({
    page,
  }) => {
    const accountPreferences = new AccountPreferencesPage(page);
    expect(await accountPreferences.isAccountPreferenceVisible()).toHaveLength(
      0,
    );
  });
});
