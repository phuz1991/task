...
export class CheckoutPage {
  ...
  fillCreditCardForm = async (
    ...
    let cardNumberFrame = await this.waitForIframeToLoad(
      CheckoutSelectors.cardNumberIframe,
    );
    let expiryDateFrame = await this.waitForIframeToLoad(
      CheckoutSelectors.expiryDateIframe,
    );
    ...
    for (let i = 0; i < 3; i++) {
      try {
        ...
        cardNumberFrame = await this.waitForIframeToLoad(
          CheckoutSelectors.cardNumberIframe,
        );
        expiryDateFrame = await this.waitForIframeToLoad(
          CheckoutSelectors.expiryDateIframe,
        );
  ...
  getSummaryText = async () => {
    const text = await this.page.waitForSelector(
      CheckoutSelectors.summaryItemName,
    );
