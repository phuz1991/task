...
export class LoginPage {
  ...
  async initialize() {
    ...
    this.passwordInput = await this.page
  ...
  checkRegisterLinkIsPresent = async () => {
    const registerLink = await this.page.waitForSelector(
      LoginSelectors.registerLink,
    );
    if (!registerLink) {
    ...
    if (linkText !== "Register a new account") {
      throw new Error(
        `Expected link text to be "Register a new account", but got "${linkText}"`,
      );
    }
    ...
    if (linkHref !== expectedHref) {
      throw new Error(
        `Expected link href to be "${expectedHref}", but got "${linkHref}"`,
      );
    }
  };
  ...
  checkIfLoginButtonIsPresent = async () => {
    ...
    if (buttonText !== "Log in") {
      throw new Error(
        `Expected button text to be "Log in", but got "${buttonText}"`,
      );
    }
    const buttonType = await this.loginButton.getAttribute("type");
    if (buttonType !== "submit") {
      throw new Error(
        `Expected button type to be "submit", but got "${buttonType}"`,
      );
    }
  };
