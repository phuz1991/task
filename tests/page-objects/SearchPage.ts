...
export class SearchPage {
    ...
    clickOnSearch = async () => {
        const searchIcon = await this.page.waitForSelector(LayoutSelectors.searchIcon)
        await searchIcon.isVisible()
        await searchIcon.click()
        await expect(this.page).toHaveTitle(/Saleor React Storefront/);
    }
    searchForQuery = async (q: string) => {
        const searchInput = await this.page.waitForSelector(SearchPageSelectors.searchInput)
    ...
    checkIfEmptyResultsMessage = async () => {
        const invalidCredentialsMessage = await this.page.waitForSelector(SearchPageSelectors.noResults, {timeout: 2000});
