...
test.describe("Basket Scenarios", () => {
    test("Basket initially is empty when no items added into checkout", async ({ page }) => {
        const basketPage = new BasketPage(page);
        const basketCount = await basketPage.getBasketCount()
        expect(basketCount).toBe(0)
    });

    test("Should be able to add first option into basket and display it properly", async ({ page }) => {
        const basketPage = new BasketPage(page);
        const basketCount = await basketPage.getBasketCount()
        expect(basketCount).toBe(0)
        ...
        const isDisabledBefore = await addToCardButtonBefore.getAttribute('disabled')
        ...
        const chooseVariantStillAppear = await productPage.getChooseVariantElement()
    ...
    test("Should be able to navigate into checkout and fill details for summary", async ({ page }) => {
        ...
        await checkoutPage.fillCreditCardForm('5252 5252 52525 52525', '12/25', '123')
